...
let nm = 20;   //nodeMargin
  ...
  //绘制节点框函数
  function drawTextItem({ startX, startY, width, height, id, line, label }) {
    let ctx = getCtx();
    // 重新开辟一个线路
    ctx.beginPath();
    // 画一个外边框
    ...
    ctx.fillText(id, startX+(width-textWidth1)/2, startY+height/2-15, textWidth1);
    ctx.fillText(line, startX+(width-textWidth2)/2, startY+height/2, textWidth2);
    ctx.fillText(label, startX+(width-textWidth3)/2, startY+height/2+15, textWidth3);
     }
...
//countTry(2,countSeq,end,judge,cfg);
    ...
    function drawSeq(node,x,y,countSeq,end,judge,cfg){
      ...
      if(fun.fun_end.includes(node)){
        ...
        drawTextItem({startX:x-nw/2,startY:y,width:nw,height:nh,id:id,line:line,label:label});
        start_x[node]=x-nw/2;  
        start_y[node]=y;
      }
...
function drawLineIfNull(startX,startY,endX,endY,length,label){
  ...
  function drawIf(node,x,y,countSeq,end,judge,cfg){
    ...
    drawTextItem({startX:x-nw/2,startY:y+count.countY-nh-nm,width:nw,height:nh,id:id,line:line,label:label});
    ...
    function drawDo(node,x,y,countSeq,end,judge,cfg){
      ...
      drawTextItem({startX:x-nw/2,startY:y+count.countY-(nh+nm)*2,width:nw,height:nh,id:id,line:line,label:label});
 ...
 // drawSwitch(1,10,10,countSeq,end,judge,cfg);
  ...
  function drawTry(node,x,y,countSeq,end,judge,cfg){
      ...
      if(cx!=0){
        ...
        drawTextItem({startX:x-nw/2,startY:y+size.countY+cy,width:nw,height:nh,id:id,line:line,label:label});
...
/* 
  ...
  function drawCFG(cfg){
      ...
      let fun=getFunStartAndFunEnd(cfg);
      console.log(fun);
      let sx=0,sy=10,count;
      for (let index = 0; index < fun.fun_start.length; index++) {
        count=drawFun(fun.fun_start[index],sx,sy,countSeq,end,judge,cfg);
        sx+=count.countX+20;
      }
  }
 */
